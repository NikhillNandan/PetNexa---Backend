<?php
/**
 * decline_booking.php - Decline a booking request
 * Deploy to: htdocs/petnexa_API/decline_booking.php
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE spa_bookings SET booking_status = 'CANCELLED', status = 'declined' WHERE booking_id = ? AND spa_id = ? AND (booking_status IN ('BOOKED', '') OR status IN ('pending', 'booked'))");
    $stmt->bind_param("ii", $booking_id, $spa_id);
    
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $response['error'] = false;
            $response['message'] = 'Booking declined successfully';
            ...
            if ($buyerRow) {
                sendFCMNotification($buyerRow['buyer_id'], 'Booking Declined',
                    'Unfortunately, your spa booking has been declined. Please try another time.',
                    'booking', $booking_id);
            }
            $buyerStmt->close();
        } else {
        ...
        $response['error'] = true;
        $response['message'] = 'Failed to decline booking: ' . $stmt->error;
    }
